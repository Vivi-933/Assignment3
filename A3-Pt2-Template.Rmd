...
```{r insight1, fig.cap="the correlation between happiness & cyclones/hurricanes hitting a country"}
...
ggplot(comb, mapping = aes(y = life_ladder, x = aware_score))+
  ...
  geom_point(data = canada, fill = 'white', color = "red", shape = 21)
```

```{r}
cor(comb$aware_score, comb$life_ladder)
...
```{r insight2, fig.height=4, fig.width=6, fig.cap="the correlation between cyclones/hurricanes hitting a country and how aware its people are of climate change"}
...
```{r insight3, fig.height=4, fig.width=6, fig.cap="the correlation between how aware the people of a country are and how happy those people are"}
```

# Conclusion and Future Work

Overall, we found that there exists a strong correlation between the impact of cyclones/hurricanes on happiness levels and also between a country's awareness of climate data.
...
- CLimate awarenes sis not avaioable for more than one year
- Cyclones dont have the countries in which they started
- Latitude and longitude are complex 
- 
