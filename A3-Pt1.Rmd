...
author: "Put your group number from MyLS here: "
...
Write a function that takes in a vector of latitude _or_ longitude in the format of these data, then outputs a positive version of the numeric part if there's an "N" or an "E", and a negative version of the numeric part if there's a "W" or "S".
...
ice_extent_yearly <- ice_extent_daily |>
    ...
    mutate(min = min(ice_extent, na.rm=TRUE),
...
aware_pop<-aware_fixed_country_match|>
    inner_join(pop_data, by = join_by(country))|>
  mutate(total_population = SP.POP.TOTL)|>
...
aware_covid<-covid_2020_match_country|>
  inner_join(aware_pop, by = join_by(country))|>
